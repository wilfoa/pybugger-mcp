...
from opencode_debugger.models.requests import EvaluateRequest
...
async def get_threads(session: SessionDep) -> ThreadListResponse:
    ...
    return ThreadListResponse(
        threads=[ThreadResponse(id=t.id, name=t.name) for t in threads]
    )
...
async def get_stack_trace(
    session: SessionDep,
    thread_id: Optional[int] = Query(None, description="Thread ID"),
